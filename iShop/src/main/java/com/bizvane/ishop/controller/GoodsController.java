...
import static com.bizvane.ishop.constant.CommonValue.bucketName;
        ...
        try {
            ...
            if (screen.equals("")) {
                if (role_code.contains(Common.ROLE_SYS)) {
                    ...
                    // list = goodsService.selectBySearch(page_number, page_size, "", search_value, null,manager_corp);
                }else {
                ...
                if (role_code.contains(Common.ROLE_SYS)) {
                    ...
                    if (role_code.equals(Common.ROLE_GM)) {
                        ...
                        // list = goodsService.selectAllGoodsScreen(page_number, page_size, "", map, null,manager_corp);
                    }  else if (role_code.equals(Common.ROLE_BM)) {
                        String brand_code = request.getSession().getAttribute("brand_code").toString();
                        brand_code = brand_code.replace(Common.SPECIAL_HEAD, "");
                        String[] brands = brand_code.split(",");
            ...
            String pathname = OutExeclHelper.OutExecl(json, goodses, map, response, request, "FAB");
...
//            }
            ...
            for (int i = 3; i < rows; i++) {
                for (int j = 0; j < clos; j++) {
                    ...
                    String goods_year = rs.getCell(j++, i).getContents().toString().trim();
...
//                    }
                    ...
                    if (cellCorp.equals("") || goods_code.equals("") || goods_name.equals("") || brand_code.equals("") || date.equals("")|| goods_year.equals("")) {
...
//                    }
                    ...
                    goods.setGoods_year(goods_year);
...
//                    goods.setMatch_goods(match_goods);
            ...
            for (Goods goods : goodses
                ...
                Goods goods1 = goodsService.getGoodsByCode(goods.getCorp_code().trim(), goods.getGoods_code().trim(), Common.IS_ACTIVE_Y);
                if (goods1 != null) {
                    goods.setId(goods1.getId());
                    result = String.valueOf(goodsService.updateExecl(goods));
                } else {
                    result = String.valueOf(goodsService.insert(goods));
                }
            }
...
//                com.alibaba.fastjson.JSONObject action_json = com.alibaba.fastjson.JSONObject.parseObject(message);
    ...
    public String getMatchFab(HttpServletRequest request) {
        ...
        try {
            ...
            } else if (role_code.equals(Common.ROLE_CM) && !corp_code_json.equals("")) {
                list = goodsService.getMatchFab(page_number, page_size, corp_code_json, search_value);
            } else {
    ...
    @RequestMapping(value = "/addOperateImages", method = RequestMethod.POST)
    @ResponseBody
    public String addOperateImages(HttpServletRequest request) {
        DataBean dataBean = new DataBean();
        String id = "";
        try {
            String jsString = request.getParameter("param");
            ...
            for (int i = 0; i < ids.length; i++) {
                Goods goods = this.goodsService.getGoodsById(Integer.parseInt(ids[i]));
                if (goods.getGoods_image() != null && !goods.getGoods_image().equals("")) {
                    ...
                    if (!goods_image.startsWith("[")) {
                        ...
                        for (int j = 0; j < array.size(); j++) {
                            JSONObject object = JSON.parseObject(array.get(j).toString());
                            object.put("image", object.get("image").toString());
                            object.put("is_public", "Y");
                            array2.add(object);
                        }
                        goods.setGoods_image(array2.toString());
                    }
    ...
    @RequestMapping(value = "/uploadOss", method = RequestMethod.POST, produces = "text/html;charset=UTF-8")
    @ResponseBody
    public String uploadOss(HttpServletRequest request) {
        DataBean dataBean = new DataBean();
        OssUtils ossUtils = new OssUtils();
        ...
        try {
            String oss_path = "";
            String server_path = path + "image/upload/base64" + Common.DATETIME_FORMAT_DAY_NO.format(new Date()) + "/" + System.currentTimeMillis();
...
//                      524,2880
                    dataBean.setId(id);
                    dataBean.setMessage("文件不得大于5M");
                    dataBean.setCode(Common.DATABEAN_CODE_ERROR);
                    return dataBean.getJsonStr();
                }
                if (StringUtils.isNotBlank(fileName)) {// 因为最后一个添加的控件没有上传相应的内容
                    FileUtils.copyInputStreamToFile(file.getInputStream(), new File(server_path + file.getOriginalFilename()));
                }
                oss_path = "Album/Vip/iShow/" + Common.DATETIME_FORMAT_DAY_NUM.format(new Date()) + ".jpg";
            } else {
                file = mRequest.getFile("vipAvatar");
...
//                      524,2880
                ...
                oss_path = "Avatar/Vip/iShow/" + Common.DATETIME_FORMAT_DAY_NUM.format(new Date()) + ".jpg";
            }


            if (!oss_path.equals("")) {
                ossUtils.putObject(bucketName, oss_path, server_path + file.getOriginalFilename());

                oss_path = "http://" + bucketName + ".oss-cn-hangzhou.aliyuncs.com/" + oss_path;

                JSONObject result = new JSONObject();
                result.put("oss_path", oss_path);
                result.put("date", Common.DATETIME_FORMAT.format(new Date()));

                LuploadHelper.deleteFile(server_path + file.getOriginalFilename());
    ...
    public String deleteOss(HttpServletRequest request) {
        ...
        try {
            ...
            String message = jsonObj.get("message").toString();
            JSONObject jsonObject = JSONObject.parseObject(message);
            String oss_path = jsonObject.getString("oss_url");
            ossUtils.deleteObject(bucketName, oss_path);
    ...
    public String getProductDetail(HttpServletRequest request) {
        ...
        try {
            ...
            Data data_time_type = new Data("product_id", product_code, ValueType.PARAM);
...
//                JSONArray array = JSON.parseArray(object.get("product_detail").toString());
