...
function getCorpList(){
	...
	oc.postRequire("post",corp_command, "", "", function(data) {
		...
		var corp_name=$("#corp_list option:selected").text();
		$("#corp").html(corp_name);
		$('.search_box .searchable-select-input').keydown(function(event){
			...
			if(event.keyCode == 13){
				...
				if(corp_code!==corp_code1){
					...
					$("#corp").html($("#corp_list option:selected").text());
					sessionStorage.setItem("sessionCorp",corp_code);//保存到本地
					insertSession(corp_code);
				}
			}
		});
		$('.search_box .searchable-select-item').click(function(){
			var corp_code1=$("#corp_list").val();
			if(corp_code!==corp_code1){
				corp_code=corp_code1;
				$("#corp").html($("#corp_list option:selected").text());
...
function insertSession(corp_code){
	...
	oc.postRequire("post","/user/getCmCorpBySession", "",param, function(data) {
		if(data.code=="0"){
			areaRanking(strReponse($("#date2").val()));
			storeRanking(strReponse($("#date3").val()));
...
function storeRanking(a){//店铺排行
	...
	param["time"]=a;
	param["store_name"]="";
	var corp_code=sessionStorage.getItem("sessionCorp");
	if(corp_code!=null&&corp_code!=undefined&&corp_code!=""){
		param["corp_code"]=corp_code;
	}
