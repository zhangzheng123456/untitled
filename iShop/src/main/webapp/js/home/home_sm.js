...
function superadditionStaff(c) {
	...
	for (var i = 0; i < c.length; i++) {
		...
		staff_list += "<tr><td style='width:96px'>" + a + "</td><td style='width:96px'>" + c[i].user_name//导购名称
			+ "</td><td style='width:96px'>" + c[i].amount //业绩
			+ "</td><td style='padding-left: 15px;'><div style='width: 50px;margin:0 auto;text-align: left'><img src='../img/contribution_"+b+".png' style='width:4px;height: 20px;float: left'>"
			+ c[i].devote_rate //贡献度
			+ "%</td></div></tr>"
	}
...
function superadditionGoods(c) {
	...
	for (var i = 0; i < c.length; i++) {
		var a = i + 1;
		goods_list += "<tr><td style='width:93px'>" + a + "</td><td style='width:93px'>"+c[i].product_code+"</td><td style='width:93px'>"+c[i].price+"</td><td>"+c[i].num_sales+"</td></tr>";
	}
	var nodata="<tr><td colSpan='4' style='padding-top:70px;'>暂无数据</td></tr>";
	if(c.length==0){
...
//var click_chart=''//折线点击
//var click_goods=''//货物点击
