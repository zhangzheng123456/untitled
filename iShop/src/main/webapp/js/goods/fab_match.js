...
function POST(a,b){
    whir.loading.add("",0.5);//加载等待框
    oc.postRequire("post","/goods/getMatchFab","0",param,function(data){
        if(data.code=="0"){
            var message=JSON.parse(data.message);
            var list=message.list;
            cout=message.pages;
...
function getVal(){
    var param={};
    param["funcCode"]=funcCode;
    oc.postRequire("post","/defmatch/list","0",param,function(data){
        if(data.code=="0"){
            console.log('页面获取数据成功！');
            ...
            for(i=0;i<msgLength;i++) {
                pageVal(img,text,oneAreaLength);
            }

        }else if(data.code=="-1"){
            console.log('页面获取数据失败！');
        }
    })
}
//数据模板
function pageVal(img,text,oneAreaLength){
    //盒子上部分+复选框
    var tempHTML1='<li class="item"><div class="boxArea"><input type="checkbox"/>';
    //内容（图片+文字）迭代生成
    var tempHTML2='<div class="oneArea"> <img src="${img}" alt=""/> <div>${text}</div> </div>';
    //盒子下部分
    ...
    var html2 = '';
        ...
        for(a=0;a<oneAreaLength;a++) {
            var nowHTML2 = tempHTML2;
            nowHTML2 = nowHTML2.replace("${img}", img);
            nowHTML2 = nowHTML2.replace("${text}", text);
            html2 += nowHTML2;
        }
