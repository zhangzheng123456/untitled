...
function superaddition(data,num){//页面加载循环
    ...
    for (var i = 0; i < data.length; i++) {
        ...
        // var time_type="";
        // if(data[i].target_type=="D"){
        //     time_type="日";
        // }else if(data[i].target_type=="Y"){
        //     time_type="年";
        // }else if(data[i].target_type=="W"){
        //     time_type="周";
        // }else if(data[i].target_type=="M"){
        //     time_type="月";
        // }
...
function GET(a,b){
    ...
    oc.postRequire("get","/userAchvGoal/list?pageNumber="+a+"&pageSize="+b
...
$("#leading_out").click(function(){
    ...
    oc.postRequire("post","/userAchvGoal/getCols","0",param,function(data){
        if(data.code=="0"){
            var message=JSON.parse(data.message);
...
$("#file_submit").click(function(){
    ...
    oc.postRequire("post","/userAchvGoal/exportExecl","0",param,function(data){
        if(data.code=="0"){
            var message=JSON.parse(data.message);
...
function UpladFile() {
    ...
    console.log(fileObj);
    var FileController = "/userAchvGoal/addByExecl"; //接收上传文件的后台地址
...
function filtrates(a,b){
    whir.loading.add("",0.5);//加载等待框
    oc.postRequire("post","/userAchvGoal/screen","0",_param,function(data){
        if(data.code=="0"){
            var message=JSON.parse(data.message);
            var list=JSON.parse(message.list);
            cout=list.pages;
