            ...
            if (list.length == 0){
                ...
                if(list.length>0 && b=="staff"){
                    ...
                    for (var i = 0; i < list.length; i++) {
                        staff_html+="<li><div class='checkbox1'><input  type='checkbox' value='"+list[i].id+"' data-phone='"+list[i].phone+"' data-storename='"+list[i].user_name+"' name='test'  class='check'  id='checkboxFourInput"
                            + i
                            + a
                            + 1
...
$("#screen_que_staff").click(function(){
    ...
    console.log(message.cache.user_codes);
    $("#screen_staff").hide();
...
$("#edit_save").click(function(){
    var param={};
    var wx="";
    if($("#wx")[0].checked==true){
        wx="Y";
    }else if($("#wx")[0].checked==false ){
        wx="N";
    }
    var sms="";
    if($("#sms")[0].checked==true){
        sms="Y";
    }else if($("#sms")[0].checked==false ){
        sms="N";
    }
    var call="";
    if($("#call")[0].checked==true){
        call="Y";
    }else if($("#call")[0].checked==false ){
        call="N";
    }
    var user_back={"call":call,"wx":wx,"sms":sms};
    param["id"]=message.cache.user_codes;
    param["user_back"]=user_back;
    oc.postRequire("post", "/user/batchBack","", param, function(data){
        console.log(data);
    })
})
$("#back_user").click(function(){
    $(window.parent.document).find('#iframepage').attr("src","/user/return_visit_list.html");
})
