...
! function(a, b) {
	...
	return b("dollar-third", [], function() {
		...
		return e.extend(d.prototype, {
			...
			request: function(b, d, e) {
				var f, h, i, j, k = 0,
					...
					n = [],
					o = [];
				for (d = c(d) ? d : [d]; m > k; k++) f = l[k], h = f.invoke(b, d), h !== g && (a.isPromise(h) ? o.push(h) : n.push(h));
		...
		return b.options.dnd = "", b.register({
			init: function(b) {
				if (b.dnd && "html5" === this.request("predict-runtime-type")) {
					...
					return e = new c(h), e.once("ready", g.resolve), e.on("drop", function(a) {
		...
		return b.register({
			init: function(b) {
				if (b.paste && "html5" === this.request("predict-runtime-type")) {
					...
					return e = new c(h), e.once("ready", g.resolve), e.on("paste", function(a) {
		...
		function c(a, c) {
			var d = this;
			d.source = c, d.ruid = a, this.size = c.size || 0, this.type = !c.type && ~"jpg,jpeg,png,gif,bmp".indexOf(this.ext) ? "image/" + ("jpg" === this.ext ? "jpeg" : this.ext) : c.type || "application/octet-stream", b.call(d, "Blob"), this.uid = c.uid || this.uid, a && d.connectRuntime(a)
		}
		return a.inherits(b, {
			constructor: c,
			slice: function(a, b) {
		...
		return b.register({
			...
			init: function(b) {
				...
				return m.queue = new c, m.stats = m.queue.stats, "html5" === this.request("predict-runtime-type") ? (d = a.Deferred(), l = new f("Placeholder"), l.connectRuntime({
					runtimeOrder: "html5"
				}, function() {
			...
			removeFile: function(a) {
				var b = this;
				a = a.id ? a : b.queue.getFile(a), a.setStatus(i.CANCELLED), b.owner.trigger("fileDequeued", a)
			},
			getFiles: function() {
		...
		function e(a, b) {
			for (var c, d = [], e = a.source, f = e.size, g = b ? Math.ceil(f / b) : 1, h = 0, i = 0; g > i;) c = Math.min(b, f - h), d.push({
				file: a,
				start: h,
				end: b ? h + c : f,
				total: f,
		...
		return d = {
			...
			d && (a.on("beforeFileQueued", function(a) {
				...
				c--
			}), a.on("uploadFinished reset", function() {
				c = 0
			}))
		...
		return b.register("FilePicker", {
			init: function() {
				...
				if (k.attr("type", "file"), k.attr("name", i.name),k.attr("id","file"), k.addClass("webuploader-element-invisible"), j.on("click", function() {
						k.trigger("click");
					}), j.css({
						opacity: 0,
						width: "100%",
				...
				f.append(k), f.append(j), e = function(a) {
			        ...
			        for(var i=0;i<obj.files.length;i++){
				        ...
				        var init_length=$(".good_imgs .parentFileBox .fileBoxUl li").length;
				        var storeAs='';
				        var imgUrl="https://products-image.oss-cn-hangzhou.aliyuncs.com";
				        var myDate = new Date();
				        var img_name=$(".good_imgs .parentFileBox .fileBoxUl").attr("data-name");
				        if($("#GOODS_CODE").val()!==''){
				        	if(init_length<20&&i<20){
					        	if(init_length>0){
					                var date=new Date().getTime();
					        		storeAs='FAB/'+$("#OWN_CORP").val()+'/'+i+date+'.jpg';//路径名称，filename的正则操作是为了接受后天数据是避免逗号冲突
						       	}else{
						       		var date=new Date().getTime();
						       		storeAs='FAB/'+$("#OWN_CORP").val()+'/'+date+'.jpg';
						       	}
						        client.multipartUpload(storeAs, file).then(function (result) {
									console.log(sessionStorage.getItem('error'));
									// if(sessionStorage.getItem('error')=='error'){sessionStorage.setItem('error','');return}
						            var storeAs='http://products-image.oss-cn-hangzhou.aliyuncs.com/'+result.name;
									swip_image.unshift(storeAs);
									// console.log( $(".good_imgs .parentFileBox .fileBoxUl").children('li.add_li'));
						            $(".good_imgs .parentFileBox .fileBoxUl").prepend('<li class="diyUploadHover"style="height: 90px"><div class="viewThumb"><img src="'+storeAs
						            	+'"></div><div class="diyCancel"></div><div class="diySuccess" ></div>\
						            	<div class="diyBar">\
										...
										<label>公开</label>\
										<em style="position: absolute; top: 0px;left: -9px;">\
						            ...
						            // $('#fileBox_'+file.id).find('.viewThumb').append('<img src="'+storeAs+'" data-name="'+storeAs+'"/>');
						        }).catch(function (err) {
						            console.log(err);
						        });
					        }
		...
		return b.register("Transport", {
			...
			send: function() {
				...
				g.sendAsBinary ? (j += (/\?/.test(j) ? "&" : "?") + d.param(f._formData), c = i.getSource()) : (b = new FormData, d.each(f._formData, function(a, c) {
					b.append(a, c)
				}), b.append(g.fileVal, i.getSource(), g.filename || f._formData.name || "")), g.withCredentials && "withCredentials" in h ? (h.open(g.method, j, !0), h.withCredentials = !0) : h.open(g.method, j), this._setRequestHeader(h, g.headers), c ? (h.overrideMimeType("application/octet-stream"), a.os.android ? (e = new FileReader, e.onload = function() {
