            ...
            (function(j){
                ...
                if(code=="isactive"){
                    if(data[i].isactive=="Y"){
                        TD+="<td><span title='是'>是</span></td>";
                    }else  if(data[i].isactive=="N"){
                        TD+="<td><span title='否'>否</span></td>";
                    }
                }else{
...
function jurisdiction(actions) {
    ...
    for (var i = 0; i < actions.length; i++) {
        if (actions[i].act_name == "add") {
            ...
            $('.action_r ul').prepend('<li id="auditParse" class="bg"><span class="icon-ishop_6-20"></span>通过审核 </li>');
        }
    }
}
...
function GET(a, b) {
    ...
    oc.postRequire("post", "/examine/search", "0", param, function (data) {
        if (data.code == "0") {
            $(".table tbody").empty();
            var message = JSON.parse(data.message);
...
function jumpBianse() {
    ...
    $(".table tbody tr").click(function () {
        ...
        $(this).toggleClass("tr");
        if (input && input.type == "checkbox" && input.name == "test" && input.checked == false) {
            input.checked = true;
            $(this).addClass("tr");
        } else if (input && input.type == "checkbox" && input.name == "test" && input.checked == true) {
    ...
    $('#add').click(function () {
        ...
        $(window.parent.document).find('#iframepage').attr("src", "/system/audit_configuration_add.html?t="+ $.now());
    });
    ...
    $('#compile').click(function(){
        ...
        if(tr.length==1){
            ...
            $(window.parent.document).find('#iframepage').attr("src","/system/audit_configuration_edit.html?t="+ $.now());
        }else if(tr.length==0){
            frame();
            $('.frame').html("请先选择");
        }else if(tr.length>1){
...
$("#delete").click(function() {
    ...
    var params= {};
    params["id"] = ID;
    oc.postRequire("post", "/examine/delete", "0", params, function(data) {
        if (data.code == "0") {
            if (value == "" && filtrate == "") {
                frame().then(function(){
...
function filtrates(a,b) {
    whir.loading.add("", 0.5);//加载等待框
    oc.postRequire("post", "/examine/screen", "0", _param, function (data) {
        if (data.code == "0") {
            var message = JSON.parse(data.message);
            var list = JSON.parse(message.list);
            cout = list.pages;
