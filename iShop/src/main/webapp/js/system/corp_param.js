...
function GET(a,b){
    ...
    oc.postRequire("get","/corpParam/list?pageNumber="+a+"&pageSize="+b
...
function jumpBianse(){
    ...
    //点击新增时页面进行的跳转
    $('#add').click(function(){
        $(window.parent.document).find('#iframepage').attr("src","/system/corp_param_add.html");
...
function POST(a,b){
    whir.loading.add("",0.5);//加载等待框
    oc.postRequire("post","/corpParam/search","0",param,function(data){
        console.log(data);
        if(data.code=="0"){
            $(".table tbody").empty();
            var message=JSON.parse(data.message);
...
$("#delete").click(function(){
    ...
    param["id"]=ID;
    console.log(param);
    oc.postRequire("post","/corpParam/delete","0",param,function(data){
        if(data.code=="0"){
            if (value == "" && filtrate == "") {
                frame();
                $('.frame').html('删除成功');
...
$("#leading_out").click(function(){
    ...
    oc.postRequire("post","/user/getCols","0",param,function(data){
        if(data.code=="0"){
            var message=JSON.parse(data.message);
...
function filtrates(a,b){
    whir.loading.add("",0.5);//加载等待框
    oc.postRequire("post","/corpParam/screen","0",_param,function(data){
        if(data.code=="0"){
            var message=JSON.parse(data.message);
            var list=JSON.parse(message.list);
            cout=list.pages;
