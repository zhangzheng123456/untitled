...
function GET(a,b){
    ...
    oc.postRequire("post","/wxTemplateContent/search","0",param,function(data){
        if(data.code=="0"){
            $(".table tbody").empty();
...
function jumpBianse(){
    ...
    $(".table tbody tr").dblclick(function(){
        ...
        if(id == "" || id == undefined){
            ...
            $(window.parent.document).find('#iframepage').attr("src","/system/setting_WeChat_template_edit.html?t="+ $.now());
        }
    });
    //点击编辑时页面进行的跳转
    ...
    $('#add').click(function(){
        ...
        $(window.parent.document).find('#iframepage').attr("src","/system/setting_Wechat_template_add.html?t="+ $.now());
    });
    //删除
...
$("#delete").click(function(){
    ...
    var param={};
    param["id"]=ID;
    oc.postRequire("post","/wxTemplateContent/delete","0",param,function(data){
        if(data.code=="0"){
            if (value == "" && filtrate == "") {
                frame().then(function(){
...
function filtrates(a,b){
    whir.loading.add("",0.5);//加载等待框
    oc.postRequire("post","/wxTemplateContent/screen","0",_param,function(data){
        if(data.code=="0"){
            var message=JSON.parse(data.message);
