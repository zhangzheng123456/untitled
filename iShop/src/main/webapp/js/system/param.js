...
function GET(a,b){
    ...
    oc.postRequire("get","/param/list?pageNumber="+a+"&pageSize="+b
...
function jumpBianse(){
    ...
    //点击新增时页面进行的跳转
    $('#add').click(function(){
        $(window.parent.document).find('#iframepage').attr("src","/system/param_add.html");
...
function POST(a,b){
    whir.loading.add("",0.5);//加载等待框
    oc.postRequire("post","/param/search","0",param,function(data){
        console.log(data);
        if(data.code=="0"){
            $(".table tbody").empty();
            var message=JSON.parse(data.message);
...
$("#delete").click(function(){
    ...
    var params={};
    params["id"]=ID;
    oc.postRequire("post","/param/delete","0",params,function(data){
        if(data.code=="0"){
            if (value == "" && filtrate == "") {
                frame();
                $('.frame').html('删除成功');
...
function filtrates(a,b){
    whir.loading.add("",0.5);//加载等待框
    oc.postRequire("post","/param/screen","0",_param,function(data){
        if(data.code=="0"){
            var message=JSON.parse(data.message);
            var list=JSON.parse(message.list);
            cout=list.pages;
