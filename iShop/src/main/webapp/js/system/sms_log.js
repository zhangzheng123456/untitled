...
function GET(a,b){
    ...
    oc.postRequire("post","/smslog/search","0",param,function(data){
        if(data.code=="0"){
            $(".table tbody").empty();
            var message=JSON.parse(data.message);
            ...
            jumpBianse();
            $("#end").attr("onclick","laydate({elem:'#end',min:'1900-01-01 00:00:00',max: '2099-12-31 23:59:59',istime: true, format: 'YYYY-MM-DD hh:mm:ss',choose:checkEnd})");
            $("#start").attr("onclick","laydate({elem:'#start',min:'1900-01-01 00:00:00',max: '2099-12-31 23:59:59',istime: true, format: 'YYYY-MM-DD hh:mm:ss',choose:checkStart})");
...
$("#delete").click(function(){
    ...
    params["id"]=ID;
    console.log(params);
    oc.postRequire("post","/smslog/delete","0",params,function(data){
        if(data.code=="0"){
            if(value==""&&filtrate==""){
                frame().then(function(){
...
$("#file_submit").click(function(){
    ...
    whir.loading.add("",0.5);//加载等待框
    oc.postRequire("post","/smslog/exportExecl","0",param,function(data){
        if(data.code=="0"){
            var message=JSON.parse(data.message);
        ...
        for(var i=0;i<filter.length;i++){
            if(filter[i].type=="text"){
                ...
                li+="<li class='created_date' id='"
                    ...
                    +filter[i].show_name
                    +"</label>"
                    +"<input style='width:260px' type='text' id='start' readonly='true' class='time_data laydate-icon' onClick=\"laydate({elem: '#start',min:'1900-01-01 00:00:00',max: '2099-12-31 23:59:59',istime: true, format: 'YYYY-MM-DD hh:mm:ss',choose:checkStart})\">"
                    +"<label style='margin: 0 32px' class='tm20'>至</label>"
                    +"<input style='width:260px' type='text' id='end' readonly='true' class='time_data laydate-icon' onClick=\"laydate({elem: '#end',min:'1900-01-01 00:00:00',max: '2099-12-31 23:59:59',istime: true, format: 'YYYY-MM-DD hh:mm:ss',choose:checkEnd})\">"
                    +"</li>";
            }
...
function checkStart(data){
    $("#end").attr("onclick","laydate({elem:'#end',min:'"+data+"',max: '2099-12-31 23:59:59',istime: true, format: 'YYYY-MM-DD hh:mm:ss',choose:checkEnd})");
};
function checkEnd(data){
    $("#start").attr("onclick","laydate({elem:'#start',min:'1900-01-01 00:00:00',max: '"+data+"',istime: true, format: 'YYYY-MM-DD hh:mm:ss',choose:checkStart})");
};
...
function filtrates(a,b){
    whir.loading.add("",0.5);//加载等待框
    oc.postRequire("post","/smslog/screen","0",_param,function(data){
        if(data.code=="0"){
            var message=JSON.parse(data.message);
            var list=message.list;
            cout=message.pages;
