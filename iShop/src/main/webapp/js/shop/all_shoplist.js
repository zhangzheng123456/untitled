...
function jurisdiction(actions){
    ...
    for(var i=0;i<actions.length;i++){
        if(actions[i].act_name=="add"){
            ...
            $('#jurisdiction').append("<li id='batchStartOrEnd'><a href='javascript:void(0);'><span class='icon-ishop_6-35'></span>开启爱秀</a></li>");
        }
    }
}
...
function jumpBianse(){
    ...
    $(".table tbody tr").dblclick(function(e){
        ...
        if(id == "" || id == undefined){
            ...
            $(window.parent.document).find('#iframepage').attr("src","/shop/all_shop_edit.html?201705190331");
        }
    })
    ...
    //点击新增时页面进行的跳转
    $('#add').click(function(){
            $(window.parent.document).find('#iframepage').attr("src","/shop/all_shop_add.html?201705190331");
        })
    //点击编辑时页面进行的跳转
    ...
    $('#qrcode').click(function(){
        ...
        whir.loading.add("",0.5);//加载等待框
        var param1={};
        var corp_code1=$(tr[length]).find(".staff").attr("data-code");
...
$("#more_down").on("click","#output_qrcode",function(){
    ...
    whir.loading.add("mask",0.5);//加载等待框
    $("#code_ma5").show();
    $("#code_ma5").css("z-index","10002");
    $("#enter5").unbind().bind("click",function(){
        $("#code_ma5").hide();
        param["is_logo"]="Y";
        whir.loading.add("",0.5);//加载等待框
    ...
    $("#code_q5").unbind().bind("click",function () {
        param["is_logo"]="N";
        $("#code_ma5").hide();
        whir.loading.add("",0.5);//加载等待框
        oc.postRequire("post","/shop/exportExecl_view ","0",param,function(data){
...
$("#more_down").on("click","#output_zip",function(){
    ...
    whir.loading.add("mask",0.5);//加载等待框
    $("#code_ma4").show();
    $("#code_ma4").css("z-index","10002");
    $("#enter4").unbind().bind("click",function(){
        param["is_logo"]="Y";
        whir.loading.add("",0.5);//加载等待框
        $("#code_ma4").hide();
    ...
    $("#code_q4").unbind().bind("click",function () {
        param["is_logo"]="N";
        whir.loading.add("",0.5);//加载等待框
        $("#code_ma4").hide();
        oc.postRequire("post","/shop/exportZip ","0",param,function(data){
...
function filtrateDown(){
    ...
    $(".isActive_select .isActive_select_down li").mousedown(function () {
        var html=$(this).text();
