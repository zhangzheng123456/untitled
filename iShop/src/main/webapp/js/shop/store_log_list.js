...
function jurisdiction(actions){
    ...
    for(var i=0;i<actions.length;i++){
        if(actions[i].act_name=="add"){
            ...
            $('#jurisdiction').append("<li id='batch_callback'><a href='javascript:void(0);'><span class='icon-ishop_6-03'></span>批量控制回访</a></li>");
        }
    }
    quanXian();
}
...
$("#more_down").on("click","#output_zip",function(){
    ...
    whir.loading.add("",0.5);//加载等待框
    oc.postRequire("post","/user/exportZip","0",param,function(data){
        if(data.code=="0"){
            var url=JSON.parse(data.message);
...
//批量离职
$("#more_down").on("click","#batch_dismiss",function(){
    var l=$(window).width();
...
$("#batch1").click(function(){
    ...
    param["id"]=ID;
    whir.loading.add("",0.5);//加载等待框
    oc.postRequire("post","/user/batchDim","0",param,function(data){
        if(data.code=="0"){
...
function GET(a,b){
    ...
    oc.postRequire("post","/shop/startStoreLogListBySearch","0",param,function(data){
            if(data.code=="0"){
            	$(".table tbody").empty();
                var message=JSON.parse(data.message);
...
$("#delete").click(function(){
    ...
    var params={};
    params["id"]=ID;
    oc.postRequire("post","/shop/storeLogDelete","0",params,function(data){
       if(data.code=="0"){
            if (value == "" && filtrate == "") {
                frame().then(function(){
        ...
        for(var i=0;i<filter.length;i++){
            if(filter[i].type=="text"){
                ...
                li+="<li class='created_date' id='"
                    ...
                    +"<input type='text' id='end"+i+"' class='time_data laydate-icon' onClick=\"laydate({elem: '#end"+i+"',min:'1900-01-01 00:00:00',max: '2099-12-31 23:59:59',istime: false, format: 'YYYY-MM-DD',choose:checkEnd})\">"
                    +"</li>";
...
function filtrates(a,b){
    whir.loading.add("",0.5);//加载等待框
    oc.postRequire("post","/shop/startStoreLogListByScreen","0",_param,function(data){
        if(data.code=="0"){
            var message=JSON.parse(data.message);
            var list=message.list;
            cout=message.pages;
...
$(document).click(function(e){
    if($(e.target).is('#user_back')||$(e.target).is('.check_select li')||$(e.target).is('.check_select .checkbox1')||$(e.target).is('.check_select .isActive_select_down')||$(e.target).is('.check_select .isActive_select_down label')||$(e.target).is('.check_select .isActive_select_down .p16')||$(e.target).is('#isactive')){
        return;
    }else{
...
function checkStart(data){
    ...
    $("#end"+num).attr("onclick","laydate({elem:'#end"+num+"',min:'"+data+"',max: '2099-12-31 23:59:59',istime: false, format: 'YYYY-MM-DD',choose:checkEnd})");
};
function checkEnd(data){
    var id=this.elem;
    var num=id.substring(id.length-1,id.length);
    $("#start"+num).attr("onclick","laydate({elem:'#start"+num+"',min:'1900-01-01 00:00:00',max: '"+data+"',istime: false, format: 'YYYY-MM-DD',choose:checkStart})");
};