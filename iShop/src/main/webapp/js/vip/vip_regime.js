...
$("#empty").click(function(){
    ...
    for(var i=0;i<input.length;i++){
        ...
        if($(input[i]).parent().hasClass("isActive_select")){
            var li=$(input[i]).next().children("li");
            for(var a=0;a<li.length;a++){
                if($(li[a]).text()=="全部"){
                    hasAll=true;
                }
            }
            input[i].value=hasAll==true?"全部":"";
        }else{
            input[i].value="";
        }
...
function GET(a,b){
    ...
    oc.postRequire("post","/vipRules/search","0",param,function(data){
            if(data.code=="0"){
                $(".table tbody").empty();
                var message=JSON.parse(data.message);
...
$('#jurisdiction').on("click","#add",function(){
    ...
    $(window.parent.document).find('#iframepage').attr("src","/vip/vip_regime_add.html?t="+ $.now());
})
//点击编辑的时候进行跳转
$('#jurisdiction').on("click","#compile",function(){
    ...
    if(tr.length==1){
        ...
        $(window.parent.document).find('#iframepage').attr("src","/vip/vip_regime_edit.html?t="+ $.now());
    }else if(tr.length==0){
            frame();
            $('.frame').html("请先选择");
    }else if(tr.length>1){
...
$("#delete").click(function(){
    ...
    params["id"]=ID;
    console.log(params);
    oc.postRequire("post","/vipRules/delete","0",params,function(data){
        if(data.code=="0"){
            if(value==""&&filtrate==""){
               frame();
               $('.frame').html('删除成功');
...
function filtrates(a,b){
    whir.loading.add("",0.5);//加载等待框
    oc.postRequire("post","/vipRules/screen","0",_param,function(data){
        if(data.code=="0"){
            var message=JSON.parse(data.message);
            var list=JSON.parse(message.list);
            cout=list.pages;
