...
function superaddition(data, num) {//页面加载循环
    ...
    for (var i = 0; i < data.length; i++) {
        ...
        + "</td><td>"
        + data[i].cardno
        + "</td><td><img src='" + data[i].image_url + "' alt=''>"
        //+ "</td><td>"
        //+ data[i].image_url
        + "</td><td>"
        + data[i].corp_name
...
function GET(a, b) {
    ...
    oc.postRequire("get", "/vipAlbum/list?pageNumber=" + a + "&pageSize=" + b, "", "", function (data) {
        if (data.code == "0") {
            $(".table tbody").empty();
            var message = JSON.parse(data.message);
...
function POST(a, b) {
    whir.loading.add("", 0.5);//加载等待框
    oc.postRequire("post", "/vipAlbum/search", "0", param, function (data) {
        if (data.code == "0") {
            var message = JSON.parse(data.message);
            var list = JSON.parse(message.list);
            cout = list.pages;
...
$("#delete").click(function () {
    ...
    params["id"] = ID;
    console.log(param);
    oc.postRequire("post", "/vipAlbum/delete", "0", params, function (data) {
        if (data.code == "0") {
            if (value == "" && filtrate == "") {
                frame();
                $('.frame').html('删除成功');
...
$("#leading_out").click(function () {
    ...
    whir.loading.add("", 0.5);//加载等待框
    oc.postRequire("post", "/goods/getCols", "0", param, function (data) {
        if (data.code == "0") {
            var message = JSON.parse(data.message);
...
function filtrates(a, b) {
    whir.loading.add("", 0.5);//加载等待框
    oc.postRequire("post", "/vipAlbum/screen", "0", _param, function (data) {
        if (data.code == "0") {
            var message = JSON.parse(data.message);
            var list = JSON.parse(message.list);
            cout = list.pages;
