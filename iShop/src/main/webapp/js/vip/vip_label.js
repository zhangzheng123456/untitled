...
function GET(a,b){
   ...
   //pageNumber:a;pageSize:b;
    oc.postRequire("post","/vipLabelsRel/search","0",param,function(data){
        if(data.code=="0"){
            $(".table tbody").empty();
            $(".table p").remove();
            var message=JSON.parse(data.message);
...
$("#delete").click(function(){
    ...
    var params={};
    params["id"]=ID;
    oc.postRequire("post","/vipLabelsRel/delete","0",params,function(data){
        if(data.code=="0"){
            if (value == "" && filtrate == "") {
                frame().then(function(){
...
$("#file_submit").click(function(){
    ...
    for(var a=1;a<allPage+1;a++){
        ...
        list_html+= '<li>'
            +'<span style="float: left">会员标签('+start_num+'~'+end_num+')</span>'
    ...
    $("#export_list_all .export_list_btn").click(function () {
        ...
        oc.postRequire("post","/vipLabelsRel/exportExecl","0",param,function(data){
            if(data.code=="0"){
                var message=JSON.parse(data.message);
...
function filtrates(a,b){
    whir.loading.add("",0.5);//加载等待框
    oc.postRequire("post","/vipLabelsRel/screen","0",_param,function(data){
        if(data.code=="0"){
            var message=JSON.parse(data.message);
            var message=JSON.parse(message.list);
            allData=message.total;
