...
function GET(a,b){
    ...
    oc.postRequire("post","/couponsLog/search","0",param,function(data){
        if(data.code=="0"){
            $(".table tbody").empty();
            var message=JSON.parse(data.message);
...
function jumpBianse(){
    ...
    //点击新增时页面进行的跳转
    $('#add').click(function(){
        $(window.parent.document).find('#iframepage').attr("src","/vip/coupon_add.html");
    })
    //点击编辑时页面进行的跳转
...
$("#delete").click(function(){
    ...
    var params={};
    params["id"]=ID;
    oc.postRequire("post","/couponsLog/delete","0",params,function(data){
        if(data.code=="0"){
            if (value == "" && filtrate == "") {
                frame().then(function(){
...
function filtrates(a,b){
    whir.loading.add("",0.5);//加载等待框
    oc.postRequire("post","/couponsLog/screen","0",_param,function(data){
        if(data.code=="0"){
            var message=JSON.parse(data.message);
            var list=JSON.parse(message.list);
            cout=list.pages;
