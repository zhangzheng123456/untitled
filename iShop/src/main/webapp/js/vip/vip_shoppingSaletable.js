...
var shop={
    ...
    data:{
        ...
        store_names:"",//存储店铺名称
        store_code:"",//存储店铺编号
        chart_area_code:"",//图表存储区域编号
        chart_area_names:"",//图表存储区域名称
        chart_store_names:"",//图表存储店铺名称
        chart_store_code:"",//图表存储店铺编号
        areaName:"",//存储店铺单独筛选名称
    ...
    allEvent:function () {
        ...
        //店铺群组单独筛选操作
        $("#areaInput,#areaInput_chart").click(function () {
        ...
        //店铺,品牌，区域筛选弹窗
        $("#storeInput,#storeInput_chart").click(function () {
            if($(this).attr("id") == "storeInput"){
                shop.data.filter_mark = "";
                var store_code = shop.data.store_code;
                var storeName = shop.data.store_names;
                var area_code = shop.data.area_code;
            }else {
                shop.data.filter_mark = "chart";
                var store_code = shop.data.chart_store_code;
                var storeName = shop.data.chart_store_names;
                var area_code = shop.data.chart_area_code;
            }
            area_code == ""?$("#area_num").val("全部"): $("#area_num").val("已选" + area_code.split(",").length + "个");
            if (store_code !== "") {
                var store_codes = store_code.split(',');
                var store_names = storeName.split(',');
                var shop_html_right = "";
        ...
        $("#area_num,#shop_area").click(function () {
            if(shop.data.filter_mark == ""){
                var area_code = shop.data.area_code;
                var area_name = shop.data.area_names
            }else {
                var area_code = shop.data.chart_area_code;
                var area_name = shop.data.chart_area_names
            }
            if (area_code !== "") {
                var area_codes = area_code.split(',');
        ...
        $("#screen_que_shop").click(function () {
            ...
            if(shop.data.filter_mark == ""){
                ...
                shop.data.chart_store_code = store_codes;
                shop.data.chart_store_names = store_names;
                $("#storeInput_chart").val(li.length!=0?"已选" + li.length + "个":"");
            }
            $("#screen_shop").hide();
            $("#pop-up-wrap").hide();
            $("#store_search").val("");
        ...
        $("#screen_que_area").click(function () {
            ...
            if(shop.data.popMark == "area"){
                if(shop.data.filter_mark == ""){
                    ...
                    $("#areaInput_chart").val(li.length!=0?"已选" + li.length + "个":"");
                }
                $("#screen_area").hide();
                $("#pop-up-wrap").hide();
                shop.data.popMark = "";
                ...
                if(shop.data.filter_mark == ""){
                    ...
                    shop.data.chart_area_code = area_codes;
                    shop.data.chart_area_names = area_names;
                }
                shop.data.shop_num = 1;
                shop.data.isscroll = false;
                $("#screen_shop .screen_content_l").unbind("scroll");
        ...
        $(".filterBtn").click(function () {
            if($(this).attr("id") == "filterBtn"){
                ...
                shop.search_value = "";
                shop.filter_list = [
                    {"screen_key":"store_code","screen_value":shop.data.store_code},
                    {"screen_key":"store_group","screen_value":shop.data.areaCode},
                ...
                shop.filter_chart_list = [
                    {"screen_key":"store_code","screen_value":shop.data.chart_store_code},
                    {"screen_key":"store_group","screen_value":shop.data.chart_areaCode}
                ];
                shop.getChartData();
                shop.getSale();
            }
        ...
        $(".emptyFilterBtn").click(function () {//清空筛选
            if($(this).attr("id") == "emptyFilterBtn"){
                ...
                shop.data.chart_area_code = "";
                shop.data.chart_area_names = "";
                shop.data.chart_store_names = "";
                shop.data.chart_store_code = "";
                shop.filter_chart_list = [
                    {"screen_key":"store_code","screen_value":""},
                    {"screen_key":"store_group","screen_value":""}
                ];
                $("#storeInput_chart").val("");
                $("#areaInput_chart").val("");
                shop.getYear();
            }
        });
    ...
    chartInit:function () {
        ...
        shop.data.chartData.map(function (arr) {
            ...
            if(type == 1){
                ...
                var newArr = [arr.discount,arr.amt_trade,arr.vip_count,arr.store_code,arr.active_rate];
    ...
    getstorelist: function (a) {//店铺接口
        ...
        var area_code = shop.data.filter_mark != "" ? shop.data.chart_area_code : shop.data.area_code;
        var _param = {};
        _param['area_code'] = area_code;
        _param['brand_code'] = shop.data.brand_code;
    ...
    getSale:function () {
        ...
        param.time_value = time_value;
        param.list = shop.filter_chart_list;
        oc.postRequire("post","/VipBusiness/storeKpiSum","",param,function (data) {
            var msg = JSON.parse(data.message);
    ...
    getTabelList:function () {
        ...
        $("#table_loading").show();
        oc.postRequire("post","/VipBusiness/storeKpi","",param,function (data) {
            if(data.code == 0){
                var msg = JSON.parse(data.message);
                ...
                switch (type){
                    case "vip" : head_concat = [
                        ...
                        tbody = list.map(function (item) {
                            var obj = {"data":[
                                {"name":item.store_id, "width":"10","classname":"ellipisis"},
                                {"name":item.store_name, "width":"10","classname":"ellipisis"},
                                {"name":item.store_group, "width":"10","classname":"ellipisis"},
                                {"name":item.store_area, "width":"10","classname":"ellipisis"},
                    ...
                    case "newVip" : head_concat = [
                        ...
                        tbody = list.map(function (item) {
                            var obj = {"data":[
                                ...
                                {"name":item.achv_rate,"per":"", "width":"6"},
                                {"name":item.vip_achv_rate,"per":"", "width":"6"}
                            ]};
                            return obj;
                        });
                    ...
                    case "actVip" : head_concat = [
                        ...
                        tbody = list.map(function (item) {
                            var obj = {"data":[
                                ...
                                {"name":item.achv_rate,"per":"", "width":"6.66"},
                                {"name":item.vip_achv_rate,"per":"", "width":"6.66"}
                            ]};
                            return obj;
                        });
                        $(".tabel_x_scroll").css("width","115%");
                    ...
                    case "vipSex" : head_concat = [
                        ...
                        {"name":"新增数量(男/女)", "width":"9", "all":shop.formatCurrency(sum.sum_vip_new_m)+'/'+shop.formatCurrency(sum.sum_vip_new_f), "avg":shop.formatCurrency(sum.avg_vip_new_m)+'/'+shop.formatCurrency(sum.avg_vip_new_f),"classname":"textCenter"},
                        {"name":"销售金额(男/女)", "width":"14", "all":shop.formatCurrency(sum.sum_amt_trade_M)+'/'+shop.formatCurrency(sum.sum_amt_trade_F), "avg":shop.formatCurrency(sum.avg_amt_trade_M)+'/'+shop.formatCurrency(sum.avg_amt_trade_F),"classname":"bgf0f0f0 textCenter"},
                        {"name":"销售件数(男/女)", "width":"9", "all":shop.formatCurrency(sum.sum_num_sales_M)+'/'+shop.formatCurrency(sum.sum_num_sales_F), "avg":shop.formatCurrency(sum.avg_num_sales_M)+'/'+shop.formatCurrency(sum.avg_num_sales_F),"classname":"textCenter"},
                        {"name":"销售笔数(男/女)", "width":"9", "all":shop.formatCurrency(sum.sum_num_trade_M)+'/'+shop.formatCurrency(sum.sum_num_trade_F), "avg":shop.formatCurrency(sum.avg_num_trade_M)+'/'+shop.formatCurrency(sum.avg_num_trade_F),"classname":"bgf0f0f0 textCenter"},
                        {"name":"折扣(男/女)", "width":"9","avg":sum.avg_discount_M+'/'+sum.avg_discount_F,"classname":"textCenter"}
                    ];
                        tbody = list.map(function (item) {
                            var obj = {"data":[
                                {"name":item.store_id, "width":"10","classname":"ellipisis"},
                                ...
                                {"name":shop.formatCurrency(item.vip_new_m)+'/'+shop.formatCurrency(item.vip_new_f), "width":"10"},
                                {"name":shop.formatCurrency(item.amt_trade_M)+'/'+shop.formatCurrency(item.amt_trade_F), "width":"10"},
                                {"name":shop.formatCurrency(item.num_sales_M)+'/'+shop.formatCurrency(item.num_sales_F), "width":"10"},
                                {"name":shop.formatCurrency(item.num_trade_M)+'/'+shop.formatCurrency(item.num_trade_F), "width":"10"},
                                {"name":item.discount_M+'/'+item.discount_F, "width":"10"}
                            ]};
                            return obj;
                        });
                        $(".tabel_x_scroll").css("width","120%");
                        break;
                    case "vipSource" : head_concat = [
                        ...
                        {"name":"已购(新增)", "width":"10", "all":shop.formatCurrency(sum.sum_wechat_vip_buy), "avg":shop.formatCurrency(sum.avg_wechat_vip_buy),"classname":"textCenter"}
                    ];
                        tbody = list.map(function (item) {
                            var obj = {"data":[
                                {"name":item.store_id, "width":"10","classname":"ellipisis"},
                                ...
                                {"name":item.store_group, "width":"10","classname":"ellipisis"},
                                {"name":item.store_area, "width":"10","classname":"ellipisis"},
                                {"name":shop.formatCurrency(item.wechat_vip_sum), "width":"10"},
                                {"name":shop.formatCurrency(item.wechat_vip), "width":"10"},
                                {"name":shop.formatCurrency(item.wechat_vip_bind), "width":"10"},
                                {"name":shop.formatCurrency(item.fans), "width":"10"},
                                {"name":shop.formatCurrency(item.new_fans), "width":"10"},
                                {"name":shop.formatCurrency(item.wechat_vip_buy), "width":"10"}
                            ]};
                            return obj;
                        });
                        $(".tabel_x_scroll").css("width","100%");
               ...
               $(".table_thead").css("padding-right",$(".table_tbodyWrap")[0].offsetWidth-$(".table_tbodyWrap")[0].clientWidth);
                ...
                switch (type){
                    case "vip" : head_concat = [
                        {"name":"会销金额", "width":"8.57","classname":"bgf0f0f0 textCenter"},
                        {"name":"会销件数", "width":"8.57","classname":"textCenter"},
                        {"name":"会销笔数", "width":"8.57","classname":"bgf0f0f0 textCenter"},
                        {"name":"会销件单价", "width":"8.57","classname":"textCenter"},
                        {"name":"会销客单价", "width":"8.57","classname":"bgf0f0f0 textCenter"},
                        {"name":"会销连带率", "width":"8.57","classname":"textCenter"},
                        {"name":"会销占比", "width":"8.57","classname":"bgf0f0f0 textCenter"}
                    ];
                        $(".tabel_x_scroll").css("width","100%");
                        break;
                    case "store" : head_concat = [
                        {"name":"业绩", "width":"10","classname":"bgf0f0f0 textCenter"},
                        {"name":"件数", "width":"10", "classname":"textCenter"},
                        {"name":"笔数", "width":"10", "classname":"bgf0f0f0 textCenter"},
                        {"name":"件单价", "width":"10","classname":"textCenter"},
                        {"name":"客单价", "width":"10","classname":"bgf0f0f0 textCenter"},
                        {"name":"连带率", "width":"10","classname":"textCenter"}
                    ];
                        $(".tabel_x_scroll").css("width","100%");
                        break;
                    case "vipNum" : head_concat = [
                        {"name":"总数", "width":"8.75","classname":"bgf0f0f0 textCenter"},
                        {"name":"新增会员", "width":"8.75","classname":"textCenter"},
                        {"name":"新客人数", "width":"8.75","classname":"bgf0f0f0 textCenter"},
                        {"name":"复购人数", "width":"8.75","classname":"textCenter"},
                        {"name":"已购(总)", "width":"8.75","classname":"bgf0f0f0 textCenter"},
                        {"name":"已购(新增)", "width":"8.75","classname":"textCenter"},
                        {"name":"活跃人数", "width":"8.75","classname":"bgf0f0f0 textCenter"}
                    ];
                        $(".tabel_x_scroll").css("width","100%");
                        break;
                    case "newVip" : head_concat = [
                        {"name":"新客人数", "width":"6","classname":"bgf0f0f0 textCenter"},
                        {"name":"新客已购", "width":"6", "classname":"textCenter"},
                        {"name":"新客消费", "width":"6", "classname":"bgf0f0f0 textCenter"},
                        {"name":"新客件数", "width":"6", "classname":"textCenter"},
                        {"name":"新客笔数", "width":"6", "classname":"bgf0f0f0 textCenter"},
                        {"name":"新客件单价", "width":"6","classname":"textCenter"},
                        {"name":"新客客单价", "width":"6","classname":"bgf0f0f0 textCenter"},
                        {"name":"新客连带率", "width":"6","classname":"textCenter"},
                        {"name":"占比/总业绩", "width":"6","classname":"bgf0f0f0 textCenter"},
                        {"name":"占比/会销", "width":"6","classname":"textCenter"}
                    ];
                        $(".tabel_x_scroll").css("width","115%");
                        break;
                    case "actVip" : head_concat = [
                        {"name":"活跃会员人数", "width":"6.66","classname":"bgf0f0f0 textCenter"},
                        {"name":"活跃业绩", "width":"6.66", "classname":"textCenter"},
                        {"name":"活跃件数", "width":"6.66", "classname":"bgf0f0f0 textCenter"},
                        {"name":"活跃笔数", "width":"6.66", "classname":"textCenter"},
                        {"name":"活跃件单价", "width":"6.66","classname":"bgf0f0f0 textCenter"},
                        {"name":"活跃客单价", "width":"6.66","classname":"textCenter"},
                        {"name":"活跃连带率", "width":"6.66","classname":"bgf0f0f0 textCenter"},
                        {"name":"占比/总业绩", "width":"6.66","classname":"textCenter"},
                        {"name":"占比/会销", "width":"6.66","classname":"bgf0f0f0 textCenter"}
                    ];
                        $(".tabel_x_scroll").css("width","115%");
                        break;
                    case "secVip" : head_concat = [
                        {"name":"复购人数", "width":"6.66", "classname":"bgf0f0f0 textCenter"},
                        {"name":"复购消费", "width":"6.66", "classname":"textCenter"},
                        {"name":"复购件数", "width":"6.66", "classname":"bgf0f0f0 textCenter"},
                        {"name":"复购笔数", "width":"6.66", "classname":"textCenter"},
                        {"name":"复购件单价", "width":"6.66", "classname":"bgf0f0f0 textCenter"},
                        {"name":"复购客单价", "width":"6.66","classname":"textCenter"},
                        {"name":"复购连带率", "width":"6.66", "classname":"bgf0f0f0 textCenter"},
                        {"name":"占比/总业绩", "width":"6.66", "classname":"textCenter"},
                        {"name":"占比/会销", "width":"6.66", "classname":"bgf0f0f0 textCenter"}
                    ];
                        $(".tabel_x_scroll").css("width","115%");
                    ...
                    case "vipAge" : head_concat = [
                        ...
                        break;
                    case "vipSex" : head_concat = [
                        {"name":"总数(男/女)", "width":"10","classname":"bgf0f0f0 textCenter"},
                        {"name":"新增数量(男/女)", "width":"10", "classname":"bgf0f0f0 textCenter"},
                        {"name":"销售金额(男/女)", "width":"10","classname":"bgf0f0f0 textCenter"},
                        {"name":"销售件数(男/女)", "width":"10", "classname":"bgf0f0f0 textCenter"},
                        {"name":"销售笔数(男/女)", "width":"10","classname":"bgf0f0f0 textCenter"},
                        {"name":"折扣(男/女)", "width":"10","classname":"bgf0f0f0 textCenter"}
                    ];
                        $(".tabel_x_scroll").css("width","100%");
                        break;
                    case "vipSource" : head_concat = [
                        {"name":"会员数量", "width":"10", "classname":"bgf0f0f0 textCenter"},
                        {"name":"新增会员", "width":"10", "classname":"textCenter"},
                        {"name":"绑卡会员", "width":"10", "classname":"bgf0f0f0 textCenter"},
                        {"name":"粉丝总数", "width":"10", "classname":"textCenter"},
                        {"name":"新增粉丝", "width":"10", "classname":"bgf0f0f0 textCenter"},
                        {"name":"已购(新增)", "width":"10", "classname":"textCenter"}
                    ];
                        $(".tabel_x_scroll").css("width","100%");
                        break;
                    case "vipCardType" : head_concat = [
                        {"name":"会员总数", "width":"6.66", "classname":"bgf0f0f0 textCenter"},
                        {"name":"已购会员", "width":"6.66", "classname":"textCenter"},
                        {"name":"会销金额", "width":"6.66", "classname":"bgf0f0f0 textCenter"},
                        {"name":"业绩占比", "width":"6.66", "classname":"textCenter"},
                        {"name":"会销件数", "width":"6.66", "classname":"bgf0f0f0 textCenter"},
                        {"name":"会销笔数", "width":"6.66", "classname":"textCenter"},
                        {"name":"会销件单价", "width":"6.66","classname":"bgf0f0f0 textCenter"},
                        {"name":"会销客单价", "width":"6.66","classname":"textCenter"},
                        {"name":"会销连带率", "width":"6.66","classname":"bgf0f0f0 textCenter"}
                    ];
                        $(".tabel_x_scroll").css("width","100%");
                        break;
                }
                head = head.concat(head_concat);
                var head_html = head.map(function (item) {
                    return '<th width="'+item.width+'%" class="'+item.classname+'">'+item.name+'</th>'
                });
                var tr = '';
                for(var i=0;i<10;i++){
                    if(i==4){
...
function create_export_list(){
    ...
    for(var a=1;a<allPage+1;a++){
        ...
        list_html+= '<li>'
            +'<span style="float: left">店铺VIP经营表('+start_num+'~'+end_num+')</span>'
