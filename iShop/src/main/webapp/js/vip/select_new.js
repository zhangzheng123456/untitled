...
function runtimeData(type) {
	...
	if (dataType == "affiliation_other") {
		html += "<p class='selected_value'>"+$(obj).find(".item_list:visible input").attr("data-name")+"</p>";
		if ($(".select_box_content_right:visible .selected_list div[data-type=" + type + "]").length == 0) {
			$(".select_box_content_right:visible .selected_list").append("<div data-type='" + type + "'>" + "<em>"+name+"</em>" + "<span  class='select_detail'>" + html + "</span><i class='delete_item icon-ishop_6-12 '></i></div>");
		} else {
		...
		item_line.map(function (index,ele) {
			...
			if ($(".select_box_content_right:visible .selected_list div[data-type=" + child_type + "]").length == 0) {
				$(".select_box_content_right:visible .selected_list").append("<div data-type='" + child_type + "'>" + name + "<span class='select_detail'>" + html + "</span><i class='delete_item icon-ishop_6-12'></i></div>");
			} else {
	...
	$(".select_box").on("click", ".item_list:visible .select_show_" + type, function () {
		...
		switch (type) {
			...
			case "staff":
				...
				if (select_affiliation.staff.code !== "") {
					var codes = select_affiliation.staff.code.split(',');
					var names = select_affiliation.staff.name.split(',');
					var html_right = "";
					for (var h = 0; h < codes.length; h++) {
						html_right += "<li id='" + codes[h] + "'>\
						            <div class='checkbox1'><input type='checkbox' value='" + codes[h] + "' name='test' class='check'>\
...
function getstafflist(a, b) {
	...
	if (b == "staff") {
		...
		_param['area_code']=select_affiliation.area.code;
		_param['brand_code']=select_affiliation.brand.code;
		_param['store_code']=select_affiliation.shop.code;
		_param['searchValue']=searchValue
	}
	_param["corp_code"] = select_corp_code;
	_param['pageNumber'] = pageNumber;
