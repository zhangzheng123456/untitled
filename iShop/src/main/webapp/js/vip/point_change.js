    ...
    if(data.length>0){
        for (var i = 0; i < data.length; i++) {
            ...
            for (var c=0;c<titleArray.length;c++){
                (function(j){
                    var code=titleArray[j].column_name,src;
                    if(code == 'bill_voucher'){
                        src = data[i][code] == '' ? '../img/goods_default_image.png' : data[i][code].split(",")[0];
            ...
            $(".table tbody").append("<tr data-state='"+data[i].bill_state+"' id='"+data[i].id+"''><td width='50px;' style='text-align: left;'><div class='checkbox'><input  type='checkbox' value='' name='test' title='全选/取消' class='check'  id='checkboxTwoInput"
                + i
                + 1
...
function jurisdiction(actions){
    ...
    for(var i=0;i<actions.length;i++){
        if(actions[i].act_name=="add"){
            ...
            sessionStorage.setItem("submit_power",true);
        }
    }
}
...
function GET(a,b){
    whir.loading.add("",0.5);//加载等待框
    oc.postRequire("post","/vipPointsAdjust/search","0",param,function(data){
        if(data.code=="0"){
            $(".table tbody").empty();
            var message=JSON.parse(data.message);
...
$('#jurisdiction').on("click","#add",function(){
    ...
    $(window.parent.document).find('#iframepage').attr("src","/vip/vip_point_changeOperate.html?t="+ $.now());
});
//点击编辑的时候进行跳转
$('#jurisdiction').on("click","#compile",function(){
    var tr=$("tbody input[type='checkbox']:checked").parents("tr");
    var state=$(tr[0]).attr("data-state");
    if(tr.length==1 && state !== "已结束"){
        var id=$(tr).attr("id");
...
$("#delete").click(function(){
    ...
    var params={};
    params["id"]=ID;
    oc.postRequire("post","/vipPointsAdjust/delete","0",params,function(data){
        if(data.code=="0"){
            if(value==""&&filtrate==""){
                frame();
                $('.frame').html('删除成功');
...
function filtrates(a,b){
    whir.loading.add("",0.5);//加载等待框
    oc.postRequire("post","/vipPointsAdjust/screen","0",_param,function(data){
        if(data.code=="0"){
            var message=JSON.parse(data.message);
            var list=JSON.parse(message.list);
            cout=list.pages;
