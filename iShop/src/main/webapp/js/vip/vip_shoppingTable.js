...
function superaddition(data,num){//页面加载循环
    ...
    for (var i = 0; i < data.length; i++) {
        ...
        $(".table tbody").append("<tr id='"+data[i].store_id+"'>"
            ...
            +"<td ><span title='"+data[i].vip_num+"'>"+data[i].vip_num+"</span></td>"
            +"<td ><span title='"+data[i].new_vip+"'>"+data[i].new_vip+"</span></td>"
            //+"<td ><span title='"+data[i].nvip_amt_trade+"'>"+data[i].nvip_amt_trade+"</span></td>"
            //+"<td ><span title='"+data[i].ovip_amt_trade+"'>"+data[i].ovip_amt_trade+"</span></td>"
            +"<td ><span title='"+data[i].fans_num+"'>"+data[i].fans_num+"</span></td>"
...
function GET(a,b){
    ...
    param.list="";
    oc.postRequire("post","/vipAnalysis/empKpi","0",param,function(data){
        if(data.code=="0"){
            $(".table tbody").empty();
            $(".table p").remove();
...
function filtrates(a,b){
    whir.loading.add("",0.5);//加载等待框
    oc.postRequire("post","/vipAnalysis/empKpi","0",_param,function(data){
        if(data.code=="0"){
            var message=JSON.parse(data.message);
            var list=message.user_list;
            cout=message.page_count;
...
function showPopup(dom){
    ...
    //获取店铺和区域
    $('#staff_name').text($(dom).parent("tr").find(".user_name").text()==""?"无":$(dom).parent("tr").find(".user_name").text());
    $('#staff_name').prop("title",$(dom).parent("tr").find(".user_name").text());
    $('#staff_code').text($(dom).parent("tr").find(".user_code").text()==""?"无":$(dom).parent("tr").find(".user_code").text());
    $('#staff_code').prop("title",$(dom).parent("tr").find(".user_code").text());
    $('#store_name_pop').text($(dom).parent("tr").find(".store_name").text()==""?"无":$(dom).parent("tr").find(".store_name").text());
    $('#store_name_pop').prop("title",$(dom).parent("tr").find(".store_name").text());
    //打开窗体
    $popup.show();
    //赋值
...
$("#file_submit").click(function(){
    ...
    for(var a=1;a<allPage+1;a++){
        ...
        list_html+= '<li>'
            +'<span style="float: left">导购VIP经营报表('+start_num+'~'+end_num+')</span>'
    ...
    $("#export_list_all .export_list_btn").click(function () {
        ...
        oc.postRequire("post","/vipAnalysis/empExportExecl","0",param,function(data){
            if(data.code=="0"){
                var message=JSON.parse(data.message);
