...
function GET(a,b){
    ...
    //pageNumber:a;pageSize:b;
        oc.postRequire("post","/CRMOperation/search","0",param,function(data){
        if(data.code=="0"){
            $(".table tbody").empty();
            $(".table p").remove();
            var message=JSON.parse(data.message);
            ...
            console.log('__________'+total+'_______'+pageNum);
...
$("#delete").click(function(){
    ...
    var params={};
    params["id"]=ID;
    oc.postRequire("post","/CRMOperation/delete","0",params,function(data){
        if(data.code=="0"){
            if (value == "" && filtrate == "") {
                frame().then(function(){
                    //var inx = $('.foot-num li').each(function () {
                    //    if($(this).find('span').attr('class')=='p-bg'){
                    //        inx = $(this).find('span').text();
                    //        console.log('当前页面是'+inx);
                    //    }
                    //});
                    param["pageNumber"]=inx;
                    param["pageSize"]=pageSize;
...
$("#file_submit").click(function(){
    ...
    whir.loading.add("",0.5);//加载等待框
    oc.postRequire("post","/CRMOperation/exportExecl","0",param,function(data){
        if(data.code=="0"){
            var message=JSON.parse(data.message);
...
function filtrates(a,b){
    whir.loading.add("",0.5);//加载等待框
    oc.postRequire("post","/CRMOperation/screen","0",_param,function(data){
        if(data.code=="0"){
            var message=JSON.parse(data.message);
            //var message=JSON.parse(message.list);
            var list=message.list;
            cout=message.pages;
            var pageNum = message.page_number;
            console.log('_____________'+pageNum);
            var total = message.total;
            $(".table tbody").empty();
            if(list.length<=0){
                $(".table p").remove();
