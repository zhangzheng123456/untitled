    ...
    if(data.length>0){
        for (var i = 0; i < data.length; i++) {
            ...
            $(".table tbody").append("<tr id='"+data[i].id+"''><td width='50px;' style='text-align: left;'><div class='checkbox'><input  type='checkbox' value='' name='test' title='全选/取消' class='check'  id='checkboxTwoInput"
                ...
                TD+
                "</tr>");
              $('.table tbody td span').each(function () {
                if($(this).text()=='material'){
                    $(this).text('材料类');
                }else if($(this).text()=='print'){
                    $(this).text('印刷类')
                }else if($(this).text()=='gift'){
                    $(this).text( '礼品类');
                }else if($(this)==''){
                    console.log('暂无内容');
                    $(this).text( '暂无');
...
function GET(a,b){
    ...
    oc.postRequire("post","/qrCode/search","0",param,function(data){
        if(data.code=="0"){
            $(".table tbody").empty();
            var message=JSON.parse(data.message);
...
$('#jurisdiction').on("click","#add",function(){
    ...
    $(window.parent.document).find('#iframepage').attr("src","/vip/vip_code_add.html");
})
//点击编辑的时候进行跳转
...
$("#delete").click(function(){
    ...
    params["id"]=ID;
    console.log(params);
    oc.postRequire("post","/qrCode/delete","0",params,function(data){
        if(data.code=="0"){
            if(value==""&&filtrate==""){
                frame();
                $('.frame').html('删除成功');
...
function filtrates(a,b){
    whir.loading.add("",0.5);//加载等待框
    oc.postRequire("post","/qrCode/screen","0",_param,function(data){
        if(data.code=="0"){
            var message=JSON.parse(data.message);
            var list=JSON.parse(message.list);
            cout=list.pages;
...
function getScreenVal(){
    ...
    if(screenArr != null&& screenArr != 'underfind' && screenArr != ''){
        ...
        for(x in screenArr){
           $(x).val(screenArr[x]);
          screenArr[x] == '材料类'?$(x).attr('data-code','material'):'';
          screenArr[x] == '印刷类'?$(x).attr('data-code','print'):'';
          screenArr[x] == '礼品类'?$(x).attr('data-code','gift'):'';
        }
