...
function GET(a,b){
    ...
    oc.postRequire("post","/vipCardType/search","0",param,function(data){
            if(data.code=="0"){
                $(".table tbody").empty();
                var message=JSON.parse(data.message);
...
$("#delete").click(function(){
    ...
    params["id"]=ID;
    console.log(params);
    oc.postRequire("post","/vipCardType/delete","0",params,function(data){
        if(data.code=="0"){
            if(value==""&&filtrate==""){
               frame();
               $('.frame').html('删除成功');
...
function filtrates(a,b){
    whir.loading.add("",0.5);//加载等待框
    oc.postRequire("post","/vipCardType/screen","0",_param,function(data){
        if(data.code=="0"){
            var message=JSON.parse(data.message);
            var list=JSON.parse(message.list);
            cout=list.pages;
