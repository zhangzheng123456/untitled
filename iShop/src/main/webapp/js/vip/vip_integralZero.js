...
$("#empty").click(function(){
    ...
    for(var i=0;i<input.length;i++){
        if(input.eq(i).css('background').indexOf('url') != -1 && !(input.eq(i).hasClass("laydate-icon"))){
            input[i].value="全部";
        }else{
            input[i].value="";
        }
...
function superaddition(data, num) {//页面加载循环
    ...
    for (var i = 0; i < data.length; i++) {
        ...
        $(".table tbody").append("<tr id='" + data[i].id + "' data-integral_name='"+data[i].integral_name+"' data-bill_no='"+data[i].bill_no+"' data-corp_code='"+data[i].corp_code+"'><td width='50px;' style='text-align: left;'><div class='checkbox'><input  type='checkbox' value='' name='test' title='全选/取消' class='check'  id='checkboxTwoInput"
            + i
            + 1
...
function GET(a, b) {
    ...
    oc.postRequire("post", "/vipIntegral/search", "0", param, function (data) {
        if (data.code == "0") {
            $(".table tbody").empty();
            var message = JSON.parse(data.message);
            var list = JSON.parse(message.list);
            cout = list.pages;
            var pageNum = list.pageNum==0?1:list.pageNum;
...
function jumpBianse() {
    ...
    $('#add').click(function () {
        ...
        $(window.parent.document).find('#iframepage').attr("src", "/vip/vip_integralZero_add.html?t="+ $.now());
    });
//点击编辑的时候进行跳转
    $('#jurisdiction').on("click","#compile",function(){
        ...
        if(tr.length==1){
            ...
            $(window.parent.document).find('#iframepage').attr("src","/vip/vip_integralZero_edit.html?t="+ $.now());
        }else if(tr.length==0){
            frame();
            $('.frame').html("请先选择");
        }else if(tr.length>1){
...
$("#delete").click(function() {
    ...
    var ID=tr.map(function(){
        return $(this).prop("id")
    }).get().join(",");
    var params= {};
    params["id"] = ID;
    oc.postRequire("post", "/vipIntegral/delete", "0", params, function(data) {
        if (data.code == "0") {
            if (value == "" && filtrate == "") {
                frame().then(function(){
...
function filtrates(a,b) {
    whir.loading.add("", 0.5);//加载等待框
    oc.postRequire("post", "/vipIntegral/screen", "0", _param, function (data) {
        if (data.code == "0") {
            var message = JSON.parse(data.message);
            var list = JSON.parse(message.list);
            cout = list.pages;
...
$("#table").on("click",".details",function(){
    var bill_no=$(this).parents("tr").attr("data-bill_no");
    var corp_code=$(this).parents("tr").attr("data-corp_code");
    var integral_name=$(this).parents("tr").attr("data-integral_name");
    var integraZero={"bill_no":bill_no,"corp_code":corp_code,"integral_name":integral_name};
    sessionStorage.setItem("integraZero",JSON.stringify(integraZero));
    $(window.parent.document).find('#iframepage').attr("src","/vip/vip_integralZero_list.html");
});
//刷新列表
