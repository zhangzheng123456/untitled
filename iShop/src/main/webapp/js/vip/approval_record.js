...
function GET(a,b){
    ...
    oc.postRequire("post","/approved/approvedList","0",param,function(data){
        if(data.code=="0"){
            $(".table tbody").empty();
...
function filtrates(a,b){
    whir.loading.add("",0.5);//加载等待框
    oc.postRequire("post","/approved/screenApprovedList","0",_param,function(data){
        if(data.code=="0"){
            var message=JSON.parse(data.message);
            var list=message.list;
            cout=message.pages;
...
$("#table").on("click",".detail",function(){
    ...
    var created_date=$(this).parents("tr").attr("data-date");
    var approved_name=$(this).parents("tr").attr("data-name");
    var approved_detail={"approved_id":approved_id,"corp_code":corp_code,created_date:created_date,approved_name:approved_name};
