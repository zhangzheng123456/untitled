...
function GET(a,b){
    ...
    oc.postRequire("post","/appIconCfg/search","0",param,function(data){
        if(data.code=="0"){
            $(".table tbody").empty();
            var message=JSON.parse(data.message);
...
function crud() {
    //点击新增时页面进行的跳转
    $('#add').click(function(){
        $(window.parent.document).find('#iframepage').attr("src","/vip/icons_add.html");
    })
    //点击编辑时页面进行的跳转
...
$("#delete").click(function(){
    ...
    var params={};
    params["id"]=ID;
    oc.postRequire("post","/appIconCfg/delete","0",params,function(data){
        if(data.code=="0"){
            if (value == "" && filtrate == "") {
                frame().then(function(){
