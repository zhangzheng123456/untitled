        ...
        for (var c=0;c<titleArray.length;c++){
            (function(j){
                ...
                if(code=="complata_rate"){
                    TD+="<td><div class='task_complete_rate'><div style='width:"+data[i][code]+"'></div></div>"+data[i][code]+"</td>";
                }else if(code=="details"){
                    TD+="<td class='staff'><a href='javascript:void(0)' data-code='"+data[i].corp_code+"' data-name='"+data[i].store_name+"' data-store='" + data[i].store_code + "'>"
                        +"查看"
                        + "</a></td>"
                }else if(code=="complete_rate"){
                    TD+='<td style="text-align: left"><div class="task_complete_rate" style="width: 130px"><div style="width:'+data[i][code]+'"></div></div>'+data[i][code]+'</td>'
                }else{
                    TD+="<td><span title='"+data[i][code]+"'>"+data[i][code]+"</span></td>";
                }
            })(c)
        }
        $("#table tbody").append("<tr id='" + data[i].corp_code + "'  data-store='" + data[i].store_code + "'>" +
...
function jurisdiction(actions) {
    ...
    for (var i = 0; i < actions.length; i++) {
        if (actions[i].act_name == "add") {
            ...
            $('#more .more_down').append("<div id=leading_out>导出</div>");
        }
    }
}
//页面加载调权限接口
...
function tableTh(){ //table  的表头
    ...
    for(var i=0;i<titleArray.length;i++){
        if(titleArray[i].show_name=="完成进度"){
            TH+="<th style='width: 200px'>"+titleArray[i].show_name+"</th>"
        }else {
...
function GET(a, b) {
    ...
    oc.postRequire("post", "/taskAnalysis/search", "0", param, function (data) {
        if (data.code == "0") {
            $(".table tbody").empty();
            var message = JSON.parse(data.message);
...
function jumpBianse() {
    ...
    $(".staff a").click(function(e){
        ...
        var store_code=$(this).attr("data-store");
        var store_name=$(this).attr("data-name");
        param["corp_code"]=corp_code;
        param["store_code"]=store_code;
        whir.loading.add("",0.5);//加载等待框
        oc.postRequire("post","/taskAnalysis/userlist","0",param,function(data){
            if(data.code=="0"){
                var message=JSON.parse(data.message);
                var list=JSON.parse(message.list);
                $('#details').show();
                $('#content').hide();
                $("#detail_store_code").val(store_code);
                $("#detail_store_code").attr("title",store_code);
                $("#detail_store_name").val(store_name);
                $("#detail_store_name").attr("title",store_name);
                ...
                if(list.length == 0){
                    ...
                    for(i=0;i<10;i++){
                        ...
                        if(i==4){
                            $($(".table #table_r tbody tr")[i]).append("<td colspan='"+len+"' style='color: #888'>暂无内容</td>");
                        }else {
                            $($(".table #table_r tbody tr")[i]).append("<td colspan='"+len+"'></td>");
                        }
                    }
                }else {
                    for(var i=0;i<list.length;i++) {
                        ...
                        $(".table #table_r tbody").append("<tr><td width='50px;' style='text-align: center;'>"
                            ...
                            + list[i].phone
                            + "</td><td>"
                            + list[i].task_complete_count
                            + "</td><td>"
                            + list[i].count
                            + "</td><td style='text-align: left'><div class='task_complete_rate' style='width: 130px'><div style='width:"+list[i].complete_rate+"'></div></div>"
                            + list[i].complete_rate
                            + "</td></tr>");
                    }
                }
                $(".table tbody tr:odd").css("backgroundColor","#e8e8e8");
    ...
    //任务详情关闭按钮
    $('#turnoff,#back_task_list').click(function(){
        $('#details').hide();
        $('#content').show();
    })
}
...
//导出拉出list
 $("#more").on("click","#leading_out",function () {
    var l = $(window).width();
...
$("#file_submit").click(function(){
    ...
    for(var a=1;a<allPage+1;a++){
        ...
        list_html+= '<li>'
            +'<span style="float: left">任务完成度('+start_num+'~'+end_num+')</span>'
    ...
    $("#export_list_all .export_list_btn").click(function () {
        ...
        oc.postRequire("post","/taskAnalysis/exportExecl","0",param,function(data){
            if(data.code=="0"){
                var message=JSON.parse(data.message);
...
function filtrates(a,b) {
    whir.loading.add("", 0.5);//加载等待框
    oc.postRequire("post", "/taskAnalysis/screen", "0", _param, function (data) {
        if (data.code == "0") {
            var message = JSON.parse(data.message);
            var list = JSON.parse(message.list);
            cout = list.pages;
