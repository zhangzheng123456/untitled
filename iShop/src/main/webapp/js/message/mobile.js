...
function jumpBianse(){
    ...
    //点击新增时页面进行的跳转
    $('#add').click(function(){
        $(window.parent.document).find('#iframepage').attr("src","/message/mobile_add.html");
    })
    //双击跳转
    $(".table tbody tr").dblclick(function(){
        var id=$(this).attr("id");
        sessionStorage.setItem("id",id);
        $(window.parent.document).find('#iframepage').attr("src","/message/mobile_edit.html");
    })
    //点击编辑时页面进行的跳转
...
$("#delete").click(function(){
    ...
    param["id"]=ID;
    console.log(param);
    oc.postRequire("post","/message/mobile/template/delete","0",param,function(data){
        if(data.code=="0"){
            if(value==""){
               frame();
               $('.frame').html('删除成功');
