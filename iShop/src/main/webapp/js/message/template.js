...
function superaddition(data,num){//页面加载循环
    ...
    if(data.length == 0){
        ...
        $(".table tbody tr:nth-child(5) td:nth-child("+n+")").append("<span style='position:absolute;left:54%;font-size: 15px;color:#999'>暂无内容</span>");
    }
...
function jumpBianse(){
    ...
    //点击新增时页面进行的跳转
    $('#add').click(function(){
        $(window.parent.document).find('#iframepage').attr("src","/message/template_add.html");
    })
    //双击跳转
...
$("#delete").click(function(){
    ...
    params["id"]=ID;
    console.log(param);
    oc.postRequire("post","/message/mobile/template/delete","0",params,function(data){
        if(data.code=="0"){
            if (value == "" && filtrate == "") {
                frame().then(function(){
