...
var boxHeight=parseFloat($('.mui-content').css('width'));
var boxWidth=parseFloat($('.mui-content').css('width'));
$('.swiper-container').css('width', boxWidth);
$('.swiper-container').css('height', boxWidth)
var LEN=0;
var HTMLVlaue=''
var fenye={//分页定义的参数
    pageNumber:1,
...
function getPage(){
   ...
   var shopMoney="";
   var shopCount="";
    if(GetRequest().shopMoney!==undefined){
       shopMoney=GetRequest().shopMoney;
    }
    if(GetRequest().shopcount!==undefined){
        shopCount=GetRequest().shopcount;
    }
    var  wdh=parseFloat($('.main_img').css('width'));
    oc.postRequire("get", "/api/shopMatch/selectById?d_match_code=" + d_match_code +"&corp_code=" + corp_code+"&user_code="+user_code+"", "0", "", function (data) {
        if (data.code == "0") {
            ...
            fenye.title=message.d_match_title;
            var creater=message.creater;
            checkUser(creater);
            if(d_match_title==undefined&&creater==undefined){
                d_match_title="搭配详情";
                $(".dapei_del").show();
            }else {
                $(".dapei_del").hide();
            ...
            var tempHTML = '<div  class="swiper-slide""> <img src="${img}" alt="" data-preview-src="" data-preview-group="1"> </div>';
            var tempHTML2 = ' <li data-target="#carousel-example-generic" data-slide-to="${num}"></li>'
            var html = '';
            var html2 = '';
            if(shopMoney==""&&shopCount==""){
                $("#shopCount").html(message.shopCount);
                $("#shopMoney").html(message.shopMoney);
            }
            if(shopMoney!==""&&shopCount!==""){
                $("#shopCount").html(shopCount);
                $("#shopMoney").html(shopMoney);
            }
            fenye.fistImg=imgArr[0];
            ...
            for(var i=0;i<imgArr.length;i++){
                ...
                var img=imgArr[i];
                (function(img){
                    // getSize(img,function(wdh,hgt,top,left){
                        LEN++;
                        var tempHTML = '<div  class="swiper-slide" style="overflow:hidden"> <img  src="${img}" alt="" data-preview-src="${previewImg}" data-preview-group="'+LEN+'"> </div>';
                        var nowHTML = tempHTML;
                        nowHTML = nowHTML.replace('${img}',img+"@800h_800w_1e_1c_1o");
                        nowHTML = nowHTML.replace('${previewImg}',img);
                        HTMLVlaue+=nowHTML;
                        if(LEN>=imgArr.length){
                            $('.swiper-wrapper').html(HTMLVlaue);
                            HTMLVlaue='';
                            LEN=0;
            ...
            $('.main_content .theDetails').html(d_match_desc)
            //评论暂无
            ...
            if(like_status == 'Y'){
                ...
                // $('.share_bottom div').eq(0).find('img').attr('src','image/icon_点赞_已点赞@2x.png');
            }else if(like_status =='N'){
                $('.bottom div').eq(0).find('img').attr('src','image/icon_点赞@2x.png');
                // $('.share_bottom div').eq(0).find('img').attr('src','image/icon_点赞@2x.png');
            }
            if(collect_status=='Y'){
                $('.bottom div').eq(1).find('img').attr('src','image/icon_收藏_已收藏@2x.png');
                // $('.share_bottom div').eq(1).find('img').attr('src','image/icon_收藏_已收藏@2x.png');
            }else if(collect_status=='N'){
                $('.bottom div').eq(1).find('img').attr('src','image/icon_收藏@2x.png');
                // $('.share_bottom div').eq(1).find('img').attr('src','image/icon_收藏@2x.png');
            }
            ...
            // $('.share_bottom div').eq(0).find('.num').text(d_match_likeCount);
            // $('.share_bottom div').eq(2).find('.num').text(d_match_commentCount);
            // $('.share_bottom div').eq(1).find('.num').text(d_match_collectCount);
            $("body").scrollTop(0);
            $(".warp").scrollTop(0);
        }else if(data.code =='-1'){
            console.log(data);
    ...
    oc.postRequire("post","/api/shopMatch/getGoodsDetails","0",param,function(data){
       if(data.code=="0"){
           if(r_match_goodsType=="fab"){
               location.href="http://"+host+"/goods/mobile_v2/goods.html?corp_code="+corp_code+"&id="+data.message+"&type=app&user_id="+user_code+"";
               $("#loading").hide();
           }
       }else if(data.code=="-1"){
           $("#loading").hide();
...
$("#main_list_share").on("click","div img",function(){
    ...
    if(share_url!==""&&share_url!=="undefined"){
        ...
        obj2.d_match_code=GetRequest().d_match_code;
        for(var key in obj2){
...
$('.editor').click(function () {
    ...
    param["url"]="http://"+host+"/goods/mobile_v2/add_new.html?d_match_code=" + d_match_code +"&corp_code=" + corp_code+"&user_id="+user_code+"&store_id="+store_id;
    console.log(param);
...
$("#omit").click(function(){
    ...
    var user_id="";
    var user_type="all";
    if(GetRequest().user_id!==undefined){
        user_id=GetRequest().user_id;
    }
    if(GetRequest().user_type!==undefined){
    ...
    var param={};
    var corp_code = GetRequest().corp_code;
    var user_code = user_id;
    var store_id=GetRequest().store_id;
    var user_type=GetRequest().store_id;
...
function toReturnShareInfo(){
    var host=window.location.host;
    var param={};
    param["d_match_image_first"]=fenye.fistImg;
    ...
    param["share_url"]=fenye.share_url;
    var param=JSON.stringify(param);
    if(fenye.d_match_image==""){
...
//权限管理
function checkUser(creater){
    var user_id = GetRequest().user_id;
    var creater_id=creater;
    if(user_id!==creater_id){
        $('.editor').css('display','none');
        $('.delete').css('display','none');
        // $('.omit').attr("type","N");
    }else if(user_id==creater_id){
...
$("#buy").click(function(){
    ...
    location.href="/goods/mobile_v2/dapei_buy.html?corp_code="+corp_code+"&productId="+productId+"&d_match_code="+d_match_code+"&user_id="+getUserCode+"&store_id="+store_id+"";
});
function getXdShareUrl() {
    ...
    oc.postRequire("post", "/api/getOpenId", "0",param, function (data) {
        if(data.code=="0"){
            fenye.share_url=data.message;

        }else if(data.code=="-1"){
            fenye.share_url="http://"+host+"/goods/mobile_v2/details_share.html?d_match_code=" + d_match_code +"&corp_code=" + corp_code+"&user_id="+user_code+"&store_id="+store_id;
        }
    });
};
function getPeopleList() {}{
    ...
    oc.postRequire("post","/api/shopMatch/getShopMatchPageViewsLog","",param, function(data){
        ...
        $("#seen_list .omit").before(html);
        $("body").scrollTop(0);
        $(".warp").scrollTop(0);
    })
};
getPeopleList();
if(document.title=="搭配详情"){
    getXdShareUrl();
}
if(document.title=="分享详情") {
//微信分享页面
    var ticket = "";
    var time = $.now();
    var isShowMask = true;  //当点击分享按钮时判断wx.config是否配置成功
    var host = location.host;//获取当前域名
    var url = location.href;
    oc.postRequire("get", "/api/wechat/jsTicket?app_id=wxc9c9111020955324", "", "", function (data) {
        ticket = data.message;
        ...
        wx.config({
            ...
            appId:"wxc9c9111020955324", // 必填，公众号的唯一标识
...
//                        alert(JSON.stringify(res));
            ...
            wx.onMenuShareTimeline({
                title: fenye.title, // 分享标题
                link: "http://" + host + "/goods/mobile_v2/link.html?d_match_code=" + d_match_code + "&corp_code=" + corp_code + "&user_id=" + user_code + "&store_id=" + store_id, // 分享链接
                desc: $('.main_content .theDetails').text(), // 分享描述
                imgUrl: fenye.fistImg, // 分享图标
                success: function () {
