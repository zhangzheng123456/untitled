...
var addProduct={
    ...
    init:function () {
        ...
        $("#brselect").click(function(){
            $(".brand_pop_parent").show();
        })
        $("#brand_pop").on("click","li",function(){
            $(this).addClass("active");
            $(this).siblings("li").removeClass("active");
            $("#brand_name").html($(this).text());
            me.brand_code=$(this).attr("data-code");
            me.page=1;
            $('#loading').show();
            me.rowno = 0;
            $('.list_box').empty();
            me.getList();
            $(".brand_pop_parent").hide();
        })
        // $("#brand_search").keyup(function () {
        //      if(event.keyCode == 13){
        //         var value=$(this).val();
        //         $("#brand_pop li").hide();
        //         $("#brand_pop li:contains('"+value+"')").show();
        //      }
        // });
        $("#brand_search").focus(function() {
            interval = setInterval(function() {
                var value=$("#brand_search").val();
                $("#brand_pop li").hide();
                $("#brand_pop li:contains('"+value+"')").show();
            }, 500);
        }).blur(function(event) {
        ...
        $("#brand_footer").click(function(){
            $(".brand_pop_parent").hide();
        })
        $(".brand_pop_parent").click(function(e){
            if($(e.target).is('.brand_pop')||$(e.target).is('.search')||$(e.target).is('#brand_search')||$(e.target).is('.brand_pop_list')||$(e.target).is('.brand_pop_list li')){
                return;
            }
            $(".brand_pop_parent").hide();
        })
        ...
        $('#select').click(function(e){
            ...
            if($(this).find("ul").css("display")=="none"){
                $(this).find("ul").show();
                $(this).find("span").addClass("rotateb");
            }else {
                $(this).find("ul").hide();
                $(this).find("span").removeClass("rotateb");
            }
        })
        $('#select ul li').click(function (e) {
            ...
            $("#select").find("span").removeClass("rotateb");
            $(this).parent("ul").hide();
        })
        ...
        $('.chooseType').click(function(){
            ...
            me.chooseId=arr;
            var labelArr=JSON.stringify(arr);
            var param={'label':labelArr,'address':'http://'+window.location.host+'/goods/mobile_v2/add_type.html?corp_code='+corp_code};
    ...
    getBrandList:function(){
        ...
        this.oc.postRequire("post","/api/shopMatch/getBrandByUser","0",param,function(data){
            var list=JSON.parse(data.message).brand;
            var html="";
            for(var i=0;i<list.length;i++){
                var style="";
                if(list[i].brand_name=="请选择"){
                    style="active";
                }
                html+="<li data-code='"+list[i].brand_code+"' class='"+style+"'>"+list[i].brand_name+"</li>"
            }
            $("#brand_pop ul").html(html);
        });
    },
    ...
    filtrateFun:function(){
        ...
        this.oc.postRequire("post","/api/fab/screenValue","0",param,function(data){
           try{
            ...
            if(waves_arr.length>0){
                $('#waves ul').html('<li>'+waves_arr.join('</li><li>')+"</li>");
            }
            if(quarters_arr.length>0) {
                ...
                if (list.length <= 0) {
                    ...
                    for (var i = 0; i < list.length; i++) {
                        ...
                        me.html += '<li class="list"> <div class="list_picture"><img src="' + list[i].goods_image +
                            ...
                            '</span></p> <p>价格：<span class="list_price">￥' +price+
    ...
    completeToApp:function (){
        ...
        if(!$('#d_match_title').val()){
            this.test('搭配名称不能为空');
            this.doAppWebRefresh('nothing',toApp);
            return
        }
        //秀搭类型不能为空
    ...
    getList:function() {
        ...
        if(this.role=='fab'){
            ...
            var row_num=$("#wx .list_box .list").length;
            ...
            me.oc.postRequire("get","/api/shopMatch/getGoodsByWx?corp_code="+corp_code+"&pageSize="+pageSize+"&store_id="+store_id+"&user_id="+user_id+"&pageIndex="+pageIndex+"&categoryId="+categoryId+"&row_num="+row_num+"&searchType="+me.searchType+"&productName="+productName+"&brand_code="+me.brand_code,'','',function(data){
                if(data.code==0){
                    var list=JSON.parse(data.message).productList;
                    if (list.length <= 0) {
