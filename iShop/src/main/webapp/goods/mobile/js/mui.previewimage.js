(function($, window) {
	...
	proto.initEvent = function() {
		var self = this;
		$(document.body).on('doubledoubletap', 'img[data-preview-src]', function() {
		...
		var laterCloseEvent = function() {
			!laterClose && (laterClose = $.later(function() {
				...
				self.loader.removeEventListener('doubletap', laterCloseEvent);
				self.scroller.removeEventListener('doubletap', laterCloseEvent);
				self.close();
			}, 300));
		};
		this.scroller.addEventListener('doubledoubletap', function() {
			if (laterClose) {
				laterClose.cancel();
				laterClose = null;
			}
		...
		this.element.addEventListener('webkitAnimationEnd', function() {
			if (self.element.classList.contains($.className('preview-out'))) { //close
				...
				self.loader.addEventListener('doubletap', laterCloseEvent);
				self.scroller.addEventListener('doubletap', laterCloseEvent);
			}
